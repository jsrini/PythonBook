            ...
            if current != None:
                if current.key == key:
                    if current.is_leaf():
                        ...
                        self.set_parents_child(rightmost,rightmost.lchild)
                        self.set_parents_child(current,rightmost)
                        rightmost.lchild = current.lchild
                        rightmost.rchild = current.rchild
                        return current
                    elif current.has_lchild():
                        self.set_parents_child(current,current.lchild)
                    else:
                        self.set_parents_child(current,current.rchild)
                    current.lchild = None
                    current.rchild = None
                    current.parent = None
                    return current
